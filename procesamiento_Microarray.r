...
#  sudo apt install libjpeg-dev  libturbojpeg0-dev
...
SDRF <- read.delim("./data/data_info.csv", sep = ",")
...
#,"normal_breast_12","tumor_breast_12")
...
df_repressed <- read.csv("../repressedGenes_breast_table.txt", header = TRUE, sep = "\t")
df_activated <- read.csv("../activatedGenes_breast_table.txt", header = TRUE, sep = "\t")

#Liempieza de datos
df_tumor <- data.frame(tumor.breast.DEG.table)

#add column 
df_tumor_tmp<-cbind(name_gen=NA, df_tumor)

#fill gen name 
for (i in 1:nrow(df_tumor_tmp)) {
  tmp_name<-df_tumor_tmp[i, 2]
  
  na<-which(df_activated == tmp_name, arr.ind = TRUE)
  nb<-which(df_repressed == tmp_name, arr.ind = TRUE)
  
  gen_a<- NULL
  gen_r<- NULL
  
  if (nrow(na) > 0) { 
    gen_a<-df_activated[na[1,1], 2]  
  }
  
    if (nrow(nb) > 0) { 
    gen_r<-df_repressed[nb[1,1], 2]  
  }
  
  if (!is.null(gen_a)) {
    df_tumor_tmp[i, 1]<- gen_a
  }
  
  if (!is.null(gen_r)) {
    df_tumor_tmp[i, 1]<-gen_r
  }
  
}

#Clean df
df_tumor_tmp <- df_tumor_tmp[!is.na(df_tumor_tmp$name_gen) & df_tumor_tmp$name_gen !="", ]

#df sorted to check gen manes
df_tumor_tmp <- df_tumor_tmp[order(df_tumor_tmp$name_gen), ]

#remove column 
df_tumor_tmp$attr_name <-NULL

#rename column 
colnames(df_tumor_tmp)[1] <- "attr_name"

library(dplyr)

# convert all values to numeric
df_tumor_tmp <- df_tumor_tmp %>%
  mutate(across(where(is.character) & !all_of("attr_name"), as.numeric))

#get avg by attr_name
df_avg_attr_name<-aggregate(df_tumor_tmp[, 2:7], list(df_tumor_tmp$attr_name), mean)

colnames(df_avg_attr_name)[1] <- "attr_name"


write.csv(df_avg_attr_name, file = "TumorBreast_table.csv")
